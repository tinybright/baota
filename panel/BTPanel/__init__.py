...
def request_check():
    ...
    if app.config['BASIC_AUTH_OPEN']:
        if request.path in ['/public','/download','/mail_sys','/hook','/config_']: return
...
@app.route('/config_',methods=method_all)
def config1(pdata = None):
    import config
    defs = ('get_token','set_token','set_token_'
            )
    return publicObject(config.config(),defs,None,pdata)
